...
class ChatbotService:
    ...
    def create_or_continue_conversation(self, session_id=None, user=None, entry_source="direct", entry_statement=None):
    ...
    def process_message(self, session_id, user_message, user=None, entry_source=None, entry_statement=None):
    ...
    def _generate_response(self, conversation, user_message):
        ...
        prompt = ChatPromptTemplate.from_messages([
            SystemMessagePromptTemplate.from_template(self.prompts.BASE_SYSTEM_PROMPT),
            MessagesPlaceholder(variable_name="chat_history"),
            HumanMessagePromptTemplate.from_template("{input}")
        ])
        
        # Create an agent with tools if we've accumulated enough context
        if len(langchain_messages) >= 6:  # System + intro + at least 2 user/assistant exchanges
        ...
        result = chain.invoke({
            "chat_history": langchain_messages[1:],  # Skip system message as it's in the prompt
            "input": user_message
        })
    ...
    def get_recent_conversations(self, user=None, limit=10):
        ...
        conversations = Conversation.objects(**query).order_by('-updated_at').limit(limit)
